...
**Build order:** all 7 milestones are done, several going further than originally scoped (matching upgraded to TF-IDF; feed UI got a 2-column grid with progressive disclosure rather than a flat list; scheduling became a real deployment instead of local cron). Milestone 7 ("Polish") is now also complete: `ingest.ts` isolates per-feed failures (a broken feed is skipped, logged, and doesn't abort the run), and cross-outlet near-duplicate dedup exists (the same story from two different sources normalizes to the same title fingerprint — outlet suffix stripped, punctuation-insensitive — so only the first-seen version is stored). Exact-URL dedup has existed since Milestone 2.
