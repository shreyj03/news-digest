...
Runs for free on [Render](https://render.com) (`render.yaml` blueprint — an API web service + a static frontend site), with [Neon](https://neon.tech) for Postgres and [cron-job.org](https://cron-job.org) (free) hitting `POST /api/tick` every 5 minutes — the scheduling engine for every user's own fetch-then-send cycle (see DECISIONS.md for why this replaced the original single daily cron once the app went multi-user, and for how it was actually deployed, via the Render and Neon MCP servers rather than by hand). `.github/workflows/tick.yml` still exists for manual/ad-hoc runs (`workflow_dispatch`) but no longer schedules itself — GitHub Actions' `schedule` event turned out to fire unreliably (effectively once a day instead of every 5 minutes) for a high-frequency cron on this repo, silently missing users' send windows; see DECISIONS.md's 2026-08-28 "Tick scheduling" entry.
...
3. Set up the real scheduling trigger on [cron-job.org](https://cron-job.org) (free): create an account, add a cronjob that sends `POST` to `<your API URL>/api/tick` every 5 minutes with an `Authorization: Bearer <FETCH_SECRET>` header (same `FETCH_SECRET` value as set in Render). GitHub Actions' own `schedule` trigger is not used for this — see DECISIONS.md's 2026-08-28 "Tick scheduling" entry for why. (Optional: in the GitHub repo's settings, add `API_URL` and `FETCH_SECRET` Actions secrets too, so `tick.yml` can still be run manually via `workflow_dispatch` for ad-hoc/debug triggers.)
