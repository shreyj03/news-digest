...
## 2026-09-11 — Gemini calls capped with a hard overall time budget

**Decision:** The 2026-09-04 retry fix (below) itself caused a real tick failure a week later: `npm run ingest` failed with three `Gemini request failed: This operation was aborted` lines and no trailing "Done." summary — the same SIGTERM-mid-run signature as the 2026-09-03 feed-fetch incident, but this time caused by Gemini's per-attempt 15s/25s timeouts firing (slow, not fast-failing) across multiple retried batches, adding up past the 60s the parent process (`runIngestAndMatchForUser` in `api/src/index.ts`) allows the whole `npm run ingest`/`npm run match` run. `summarize.ts`'s `callGemini` now takes an absolute `deadline` (not a duration): each attempt shrinks its own timeout to whatever's left of the shared budget, skips entirely once under 3s remains, and skips a retry if there isn't time for one; `summarizeArticles`/`summarizeTopicRecaps` compute a 30s deadline up front (well under the 60s parent budget, leaving room for feed fetching/DB work) and stop starting new batches once it passes, logging how many were skipped. Also checked whether `ADMIN_ALERT_EMAIL` alerts ever reach other users — they don't: `sendAdminAlert` (api/src/index.ts) always sends to the single `ADMIN_ALERT_EMAIL` address regardless of which user's tick failed, never to `user.email`; confirmed via every `sendEmail(...)` call site in the file.

**Why:** summarization/recaps are documented as "best-effort... never block or fail a run" (see summarize.ts's top comment), but that was only true when Gemini failed *fast* — a slow/hanging response was never accounted for, and the very fix that made 503s survivable (retrying) made *this* failure mode worse by adding more wall-clock time per batch. A per-request timeout alone can't fix this because the danger is cumulative across batches and retries, not any single call; only a shared, shrinking deadline threaded through every attempt guarantees the total time spent on AI features has a hard ceiling regardless of how many articles/topics there are or how unreliable Gemini is that day. Verified live: a 60-article (3-batch) run that hit failures on every attempt still returned in exactly 30.0s (the budget), not 60+.

**Alternatives considered:**
- Just lowering the per-attempt timeout or dropping the retry — rejected: doesn't fix the actual defect (no cap on *total* time across batches), just narrows the window until the next multi-batch run hits it again.
- A `Promise.race` against a single run-wide timer that hard-aborts everything at once — rejected as more complex for no real benefit over checking a shared deadline at each natural decision point (before an attempt, before a retry, before a batch); the chosen approach fails closed at the same granularity with far less code.
- Applying the same deadline pattern to `api/src/ai.ts`'s `suggestKeywords` — held off: it runs inside an HTTP request/response cycle, not a hard-killed child process, so its worst case (~21.5s) is a slow request, not a crashed run; revisit only if that latency itself becomes a real problem.

## 2026-09-04 — Gemini calls retry once and switched off flash-lite-latest

**Decision:** AI summaries and topic recaps had silently stopped completely — zero of the last 327 articles got an `ai_summary`, and no `topic_recaps` row had been written since 2026-09-03 11:56 UTC, despite all 3 users' ticks running successfully every day since. Live-tested the real `GEMINI_API_KEY` directly against `gemini-flash-lite-latest` (the model both `ingest/src/summarize.ts` and `api/src/ai.ts` call): the key itself is valid, but the model returned `503 UNAVAILABLE` ("currently experiencing high demand") on roughly half to two-thirds of requests, with zero retry logic to absorb that. Two fixes: (1) both files now retry once (2 attempts total, 1.5s pause) on a `429`/`5xx` response before giving up on a batch; (2) switched the model from `gemini-flash-lite-latest` to `gemini-flash-latest` — a clean side-by-side test against the same key at the same moment showed flash-latest succeeding far more often.

**Why:** the DB showed a telling shape, not a hard toggle — success declined gradually (100% in some id buckets, then 72%, then 49%, then a flat 0% for the last ~3 days) rather than snapping from all-working to all-broken, which is what you'd expect from a transient-overload problem getting worse over time on Google's side, not from the API key being revoked or a code regression (either of those would have been a clean before/after cut, and would have logged a non-retryable error like 401/403/404 instead of 503). `callGemini`'s single-attempt design was reasonable when this was built (2026-08-31) and worked fine for ~2 days; it just had no way to survive the model tier becoming unreliable. A single retry with a short pause was enough in testing to turn a 3-article batch from 0/3 summarized (both attempts hit 503) to 3/3 on a clean run once combined with the model switch.

**Alternatives considered:**
- Retry-only, keep flash-lite-latest — considered, but a controlled side-by-side test (same key, same moment) still showed flash-latest meaningfully more available; not worth leaving a known-worse model in place when the swap is a one-line, same-free-tier change.
- Exponential backoff / more than one retry — rejected as unnecessary complexity for now: one retry already turned a working smoke test around, and `ingest.ts`'s parent process only has a 60s exec budget total (see 2026-09-03 entry above) to spend across every batch, so retries need to stay cheap.
- Falling back to a second model automatically on failure (flash-lite-latest → flash-latest → …) instead of just switching primaries — rejected as overbuilt for a two-model, low-volume app; revisit only if flash-latest itself later shows the same overload pattern.

## 2026-09-03 — Ingest feed fetches parallelized; alert dedup keyed off first line only

**Decision:** Two related fixes after a real production incident: the healthcheck watchdog fired, and two "tick failed" admin alerts arrived 5 minutes apart, both `fetch: Command failed: npm run ingest` with 4 "Status code 429" lines from Google News RSS and no trailing "Done." summary. `ingest.ts` now fetches all of a user's feeds concurrently (`Promise.allSettled`) instead of one at a time in a `for` loop, and `Parser` gets an explicit 20s per-request timeout plus a real browser `User-Agent` header (rss-parser's own default per-request timeout is 60s — identical to the 60s the API allows the whole `npm run ingest` child process in `runIngestAndMatchForUser`). Separately, `/api/tick`'s same-day alert dedup (`alertedToday`) now keys off just `error.split("\n")[0]` instead of the full error string.

**Why:** sequential fetching meant four slow/rate-limited feed requests could together exceed the outer 60s exec timeout even though each one failed fast and cleanly on its own — the process got SIGTERM'd mid-run before it could log its own "Done." summary, which is why the alert showed the four per-feed failures but nothing after them. Parallelizing bounds total wall-clock time by the single slowest feed instead of their sum; inserts stay sequential afterward so the existing cross-outlet dedup `Set` (which two different feeds carrying the same story both need to see consistently) isn't touched. Separately, the two alerts 5 minutes apart for what was really one ongoing failure happened because the dedup key included the full shelled-out stderr, which embeds the child process's PID (`(node:693)` vs. `(node:729)`) — a value that's different on every single retry by construction, silently defeating the same-day dedup for this entire class of error. The first line of the error (`Command failed: npm run ingest`) is stable across retries and is what actually identifies the failure; the full text is still shown in the alert body itself, just not used as the key.

**Alternatives considered:**
- Raising the exec timeout instead of parallelizing — rejected as treating the symptom, not the cause: it would still mean total time scales with feed count, and doesn't fix the underlying reason a single ingest run could be slow (sequential I/O) in the first place.
- Retrying 429s with backoff — held off for now: no evidence yet that a retry would succeed within budget (Google News' rate limiting on cloud-hosting IPs isn't obviously transient at the per-request level), and it would add complexity for an uncertain payoff; parallelizing is the fix that's actually verified to matter here since the failures observed were fast per-feed rejections, not hangs.
- A content hash or normalized-error-type enum for the alert dedup key, instead of just the first line — rejected as overbuilt: the first line already reliably matches for the one real case seen (a full shelled-out command failure), and a fancier scheme can wait until it's shown to actually be needed.

## 2026-08-31 — Automated tests + extracting pure functions for testability

**Decision:** Added Vitest to both `api/` and `ingest/` (`npm test` in each). Rather than testing through the full app (which would mean booting Express/`app.listen()` or running match.ts's `main()` against a real database), extracted the pure, side-effect-free logic each package already had embedded in larger files into their own modules: `api/src/schedule.ts` (`getLocalTimeAndDate`, `minutesSinceMidnight`, `withinWindow`, the tick window constants — previously private functions inside `index.ts`) and `ingest/src/textUtils.ts` (`normalizeTitle` from `ingest.ts`, `escapeRegExp`/`countOccurrences` from `match.ts`). `index.ts`/`ingest.ts`/`match.ts` now import from these instead of defining the logic inline — no behavior change, pure extraction. 22 tests total: tick-window edge cases (exactly-at-target, just-inside, just-outside, and the midnight-wraparound case already called out in `withinWindow`'s own comment), TF-IDF's word-boundary matching (the "OPT" vs. "Options" case from `countOccurrences`'s own comment), cross-outlet title normalization, and the keyword-suggestion fallback. `tsc -p tsconfig.build.json` (a new build-only tsconfig extending the base one) excludes `*.test.ts` from what actually ships to `dist/`, while the base `tsconfig.json` (used by `tsc --noEmit`) still typechecks the tests themselves.

**Why:** the user asked for tests specifically around "matching/scoring and the tick-window logic" — exactly the two areas of this app where a subtle logic bug wouldn't show up as a crash, only as silently wrong output (an article that should match doesn't, or a digest that should send at 7am doesn't) — which is exactly what happened with the tick-window's midnight-wraparound math and is worth locking down. Testing through the real app (spinning up Express, connecting to Postgres) would make these tests slow, flaky in CI, and coupled to infrastructure that has nothing to do with the logic being tested; extracting pure functions first is what makes fast, deterministic unit tests possible at all here.

**Alternatives considered:**
- Jest instead of Vitest — rejected: both packages are ESM (`"type": "module"`) using `tsx` already, and Vitest's ESM/TS support works with zero extra config, whereas Jest's ESM support needs more setup (`ts-jest`/`babel-jest`, `NODE_OPTIONS=--experimental-vm-modules`) for the same result.
- Testing `index.ts`/`match.ts` as-is via mocking `express`/`pg` — rejected: would test the mocks more than the logic, and doesn't solve `match.ts`'s `main()` running unconditionally at import time either.
- A single shared test-utils package across `api/` and `ingest/` — rejected, consistent with this project's existing choice to keep those two npm packages independent rather than introduce a shared internal package (see the multi-user scheduling entry below).

## 2026-08-31 — Daily topic recaps + top-story picks (Gemini Flash)

**Decision:** Added a `topic_recaps` table (`topic_id`, `date`, `recap`, `top_article_id`, `PRIMARY KEY (topic_id, date)`). After each `match.ts` run, for every topic with at least one article matched *today* (same "today" `buildFeed` already uses — server clock, `CURRENT_DATE`), its top 6 matches are batched to Gemini (new `summarizeTopicRecaps` in `summarize.ts`) asking for one thing per topic: a neutral 2-3 sentence recap of what's happening, and which single article (if any) is the day's biggest story for that topic. Upserted on `(topic_id, date)`, so re-running `match.ts` the same day (e.g. clicking "Fetch news" twice) refreshes the recap rather than duplicating it. `buildFeed` (shared by `GET /api/feed` and the digest email) now also fetches that day's recap and marks the picked article with `top_story: true`; both the web feed (an italic paragraph above the article list, a small "Top story" tag on the flagged article) and the digest email (same, styled to match) show it. No recap when `buildFeed` falls back to stale/older matches — a recap only ever describes one specific day, so a "most recent matches, any date" fallback has no single day to summarize.

**Why:** the user asked for exactly this — "one AI-written summary at the top of each topic" plus "an LLM pass to flag the day's biggest story" — as two separate ideas, but they're naturally one Gemini call and one table: the model is already reading all of a topic's matched headlines to write the recap, so asking it to also point at the most significant one is nearly free (no extra request). Generating once per `match.ts` run (not once per page view) matches the project's established pattern for `ai_summary` and the digest email itself: content is computed once, shared by every reader, not regenerated per request.

**Alternatives considered:**
- Re-ranking `topic_articles.score` itself (TF-IDF + an AI importance multiplier) instead of a separate top-story pick — rejected: would blur relevance (does this article match the topic's keywords) with significance (is this the biggest story), two different questions; keeping the AI pick as metadata alongside the existing score (which stays untouched) means a failed/unset Gemini call can never change which articles show up or in what order, only whether one gets a badge.
- Generating the recap on-demand inside `GET /api/feed`/the email send, instead of during `match.ts` — rejected: would mean an extra Gemini call (with real latency and failure risk) on every page load for the demo account, which any anonymous visitor can trigger repeatedly; generating once during the already-scheduled fetch avoids that entirely.
- A flat 15s timeout, matching `ai_summary`'s — raised to 25s for this call specifically after two real requests during testing hit the 15s limit before a third succeeded; picking a "most significant" article is a heavier reasoning task than one-sentence-per-item summarization and needed the extra headroom in practice.

## 2026-08-31 — Auto-expanded topic keywords (Gemini Flash)

**Decision:** `POST /api/topics` now asks Gemini (new `api/src/ai.ts`, `suggestKeywords(name, userTypedKeywords)`) to suggest up to 8 keywords/phrases for a new topic, merged with whatever the user already typed (deduplicated, user's own casing kept). If Gemini is unset or fails, and the user typed nothing, falls back to a plain word-split of the topic name (`defaultKeywordsFromName` — lowercase, stopwords dropped) rather than leaving keywords empty. Scoped to topic *creation* only, not edits — `PUT /api/topics/:id` still saves exactly what the user typed, unchanged, since an edit is an explicit "I know what I want" action that shouldn't get keywords silently appended.

**Why:** match.ts's TF-IDF scoring skips any article that scores zero across *all* of a topic's keywords — a topic created with no keywords typed (previously entirely possible; the field was optional) could never match a single article, ever, with no error or warning anywhere. This wasn't a hypothetical: it's the exact bug this fixes. The user asked for this as "keyword expansion for better recall," and it turned out to also be the fix for a real dead-end case, not just an enhancement. Verified against the live API before shipping: `suggestKeywords("OKLO", [])` came back with `["Oklo", "Sam Altman", "nuclear energy", "fission", "SMR", ...]` — literal terms that would actually appear in headlines, not vague synonyms — because the prompt explicitly warns that matching is literal word/phrase matching, not semantic.

**Alternatives considered:**
- Also auto-expanding on edit — rejected: edits are a deliberate, already-informed action (the user is looking at their existing keywords when they change them), and silently appending AI suggestions there risks surprising someone who trimmed their list on purpose.
- Blocking topic creation until the Gemini call resolves with no fallback — rejected: would make topic creation depend on a third-party API's uptime for a feature (better matching) that's valuable but not essential; the plain word-split fallback means a topic is never stuck with zero keywords regardless of Gemini's availability.

## 2026-08-31 — Reliability: admin alert email + external dead-man's-switch ping

**Decision:** Two additions to `/api/tick`, both optional (unset = today's exact behavior, no change): (1) `ADMIN_ALERT_EMAIL` — when a user's fetch or send fails inside a tick run, an alert email goes out via the existing Brevo `sendEmail` helper, de-duplicated in-memory to at most one per `(user, local day, error message)` so a persistent failure doesn't re-alert every 5 minutes for as long as it lasts. (2) `HEALTHCHECK_PING_URL` — a fire-and-forget `fetch` at the end of every `/api/tick` call (regardless of whether any user was actually due), meant to be pointed at a free dead-man's-switch service like healthchecks.io: as long as cron-job.org keeps calling `/api/tick` on schedule, the ping keeps arriving; if cron-job.org ever stops calling it *at all*, nothing inside this app can notice that on its own, but a service watching for the ping's absence can.

**Why:** the user asked for exactly this gap — "nobody finds out until a digest doesn't arrive." The two mechanisms cover two genuinely different failure modes: (1) covers `/api/tick` getting called but something inside it failing (Brevo down, a DB error, a feed erroring out); (2) covers `/api/tick` not getting called at all, which is precisely the failure that caused the original missed 7am digest this project already fixed once (GitHub Actions' `schedule` trigger silently going quiet — see the 2026-08-28 "Tick scheduling" entry). (1) is fully self-contained code; (2) needs an external watcher by definition, since a process that's stopped being invoked can't page anyone about its own absence — `HEALTHCHECK_PING_URL` is the hook, but actually setting up the healthchecks.io account is a step for the project owner, not something achievable by an API key alone (see README).

**Alternatives considered:**
- A DB column (`users.last_alert_date`) instead of an in-memory `Set` for de-duplication — rejected for now: the in-memory version resets on a Render process restart (free tier restarts periodically), meaning a rare extra duplicate alert in the worst case, which is an acceptable cost for avoiding schema growth for a minor throttle — consistent with this project's existing bar (e.g. the tick-scheduling entry's own alert-dedup discussion).
- Relying solely on cron-job.org's own built-in failure notifications instead of a separate healthchecks.io ping — cron-job.org can alert on a non-2xx response, which is complementary and worth also turning on in its dashboard, but doesn't cover the case where cron-job.org's own scheduler itself stops firing (exactly what happened to GitHub Actions) — only an independent, differently-timed watcher can catch that.

## 2026-08-31 — AI article summaries: Gemini Flash (free tier)

**Decision:** Added a display-only `ai_summary TEXT` column on `articles`, populated once per newly-inserted article by a new `ingest/src/summarize.ts`. After each ingest run, every article inserted that run is batched (25 at a time, one request per batch) to Gemini's small/fast tier (`gemini-flash-lite-latest`, via Google AI Studio's free API — `GEMINI_API_KEY`) asking for one short, neutral sentence per article, using structured output (`responseSchema`) so the response is parsed as JSON rather than scraped from free text. The `-latest` alias is used on purpose rather than a pinned version: the first version tried (`gemini-2.0-flash`) turned out to already 404 against the live API by the time this shipped — confirmed via the API's own `ListModels` endpoint, which showed it superseded by a longer list of newer names — so pinning any specific version string risks the exact same silent breakage later. Only title + the existing RSS `contentSnippet` are sent (no full-article fetch/scrape) — same information the app already stores, just condensed into a cleaner sentence than the raw RSS fragment. `ai_summary` is deliberately a *new* column, not an overwrite of the existing `summary` column: `summary` (the raw RSS snippet) still feeds match.ts's TF-IDF scoring and the "why did this match" keyword highlighting exactly as before, so a slow/failed/unset summarization call can never change what matches or why. Missing `GEMINI_API_KEY`, a request timeout (15s), a non-200 response, or a malformed reply all fail open — caught, logged, `ai_summary` stays null — never fails or blocks the ingest run itself. Surfaced in both the web feed (a small italic line under the headline) and the digest email. Verified against the live Gemini API with real sample headlines before deploying (correct, well-formed one-sentence summaries came back), not just via typecheck.

**Why:** the user asked for AI-generated article summaries "for free." Summarizing at ingest time, once per article, matches the project's existing "articles are a shared, deduplicated pool" design (CLAUDE.md/DECISIONS.md's multi-user entry) — one summarization call per article ever, regardless of how many users' topics later match it, rather than re-summarizing per user per view. Batching all of one run's new articles into as few requests as possible (not one request per article) keeps free-tier rate limits from being a practical concern at this project's scale.

**Alternatives considered:**
- Groq (Llama 3.1/3.3 8B) — the original pick for its very generous free-tier rate limits, but Groq discontinued its free tier before this shipped; Gemini Flash (Google AI Studio) is the current no-cost option with a comparable "small, fast, hosted" model and its own generous free daily quota.
- A local/self-hosted small model (e.g. via Ollama) — rejected: the API runs on a free Render web service with no GPU and limited RAM/CPU, so hosting inference there isn't practical; an API-based free tier is the only option that fits the project's existing "free, hosted" deployment model.
- Summarizing full article text (fetched/scraped from each article's own page) instead of just the RSS title+snippet — rejected for now: scraping arbitrary publisher pages is fragile (paywalls, JS-rendered content, inconsistent markup) and adds real latency/failure surface to every ingest run; title+snippet is already on hand and good enough for a one-sentence teaser. Worth revisiting only if the snippet-based summaries turn out to be low-value in practice.
- Overwriting the existing `summary` column with the AI-generated text instead of adding `ai_summary` — rejected: `summary` is a scoring input (TF-IDF via match.ts), and coupling a best-effort, sometimes-null AI call to that would make scoring quality depend on a third-party API's uptime.
