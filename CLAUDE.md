...
**Post-launch improvements (2026-08-27):** ticker sparklines (7-day trend line, Yahoo's already-fetched data just wasn't kept before); per-outlet diversity cap on the feed (max 3 per outlet, derived from the title suffix — not the `source` column, which is the feed name and would've capped whole topics almost to nothing, caught before shipping); a "why did this match" tooltip on the signal meter listing which keywords actually hit; a 7-day history view (`GET /api/feed?date=`, UTC-based date pills in the UI); and a morning digest email scaffold (`/api/fetch?email=1` via Resend) that's coded and wired into the scheduled workflow but inactive until `RESEND_API_KEY`/`DIGEST_EMAIL_TO` are set on the API service.
