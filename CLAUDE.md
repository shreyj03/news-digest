...
**Post-launch improvements (2026-08-27):** ticker sparklines (7-day trend line, Yahoo's already-fetched data just wasn't kept before); per-outlet diversity cap on the feed (max 3 per outlet, derived from the title suffix — not the `source` column, which is the feed name and would've capped whole topics almost to nothing, caught before shipping); a "why did this match" click/tap-to-reveal caption on the signal meter listing which keywords actually hit (shipped as hover-only `title` first — fixed after the user reported not seeing it, since native tooltips don't exist on touch devices); a 7-day history view (`GET /api/feed?date=`, UTC-based date pills in the UI); and a morning digest email (`/api/fetch?email=1` via Resend) — active and confirmed delivered to the user's inbox once `RESEND_API_KEY`/`DIGEST_EMAIL_TO` were set on the API service.
